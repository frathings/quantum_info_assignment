...
\ket{a, a \oplus b}$, implementing the same boolean function of a XOR, and the \textit{Toffoli},
which implements $\ket{a,b,c} \to \ket{a, b, c \oplus ab}$,
a reversible controlled update.
...
Execution time was measured for $N \in \{15, 55, 65, 133\}$ (corresponding to $L = 4, 6, 7, 8$
...
There are some deviations which can be explained as finite-size effects and overheads of the implementation. 
\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{shor_scaling.png}
    \caption{Execution time vs.\ number of bits $L = \lceil\log_2 N\rceil$ on a
    semi-log scale. Are included 10 executions to estimate the variability.}
    \label{fig:scaling}
\end{figure}
