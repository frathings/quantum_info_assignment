...
analog of AND followed by XOR. Since any classical boolean function can be
decomposed into XOR and AND, CNOT and Toffoli are sufficient to implement any
classical computation reversibly.

The \textit{CARRY gate} computes $c_i = a_i b_i \oplus c_{i-1}(a_i \oplus b_i)$
...
The \textit{SUM gate} implements $a_i \oplus b_i \oplus c_{i-1}$ which is a chain of
...
$a + b$ if no overflow occurred, and $a + b - N$ otherwise; it suffices to detect
which case applies and correct accordingly.

The circuit proceeds in five steps:

\textbf{(1) Add.} Compute $b \leftarrow a + b$ using the plain adder. The overflow
...
After applying the inverse QFT to the control register, the measurement histogram peaks at multiples of $2^{n_x}/r$. For $N=33$, $a=7$,
